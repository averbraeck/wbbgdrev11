...
/**
 * Text animation for the Bridge. Separate class to be able to turn it on and off...
 * <p>
 ...
 */
public class BridgeTextAnimation extends TextAnimation
{
    /** */
     ...
     */
    public BridgeTextAnimation(final Locatable source, final String text, final float dx, final float dy,
    ...
    public TextAnimation clone(final Locatable newSource, final OTSSimulatorInterface newSimulator)
            throws RemoteException, NamingException
    {
        return new BridgeTextAnimation(newSource, getText(), getDx(), getDy(), getTextAlignment(), getColor(), getFontSize(),
                newSimulator);
    }
